...
encodeRequest (ReqStamp s n)    = [M.ObjectWord 2, M.ObjectMap [(M.ObjectStr "stamp_set", M.ObjectWord s),(M.ObjectStr "stamp_index", M.toObject n)]]
encodeRequest (ReqYesNo q)      = [M.ObjectWord 500, M.ObjectMap [(M.ObjectStr "question", M.ObjectStr q), (M.ObjectStr "listing", M.ObjectBool False)]]
encodeRequest (ReqSelect q as)  = [M.ObjectWord 502, M.ObjectMap [(M.ObjectStr "question", M.ObjectStr q),(M.ObjectStr "options", M.toObject as), (M.ObjectStr "listing", M.ObjectBool False)]]
encodeRequest (ReqTask ttl cls) = [M.ObjectWord 504, M.ObjectMap [(M.ObjectStr "title", M.ObjectStr ttl),(M.ObjectStr "closing_type", M.ObjectWord (if cls then 1 else 0))]]
