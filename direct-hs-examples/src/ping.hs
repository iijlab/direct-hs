...
handleCreateMessage client (D.Txt "stamp",_,room,_) =
    void $ D.sendMessage client (D.Stamp 3 1152921507291204297 (Just "スタンプ！")) (D.talkId room)
handleCreateMessage client (D.Txt "yesno",_,room,_) =
    void $ D.sendMessage client (D.YesNoQ "お元気ですか？") (D.talkId room)
handleCreateMessage client (D.Txt "select",_,room,_) =
    void $ D.sendMessage client (D.SelectQ "好きなクワガタは？" ["ヒラタ", "ミヤマ", "オオ"]) (D.talkId room)
...
handleCreateMessage client (D.Txt "whoareyou",_,room,_) = do
    ...
    void $ D.sendMessage client (D.Txt $ "私は" `T.append` D.displayName me `T.append` "です") (D.talkId room)
...
handleCreateMessage client (D.Txt "domains",_,room,_) = do
    ...
    void $ D.sendMessage client (D.Txt (ans `T.append` "があります。")) (D.talkId room)
...
handleCreateMessage client (D.SelectA "好きなクワガタは？" _ ans,_,room,_) =
    void $ D.sendMessage client (D.Txt (ans `T.append` "が好きなんですね。")) (D.talkId room)
...
handleCreateMessage client (D.Location addr _url,_,room,_) =
    void $ D.sendMessage client (D.Txt (addr `T.append` "にいるのね。")) (D.talkId room)
handleCreateMessage client (D.Other txt,_,room,_) =
    void $ D.sendMessage client (D.Txt ("Other" `T.append` txt)) (D.talkId room)
